...
module RailsDeprecationLogAnalyser
  module Classifier
    RSpec.describe MimeTypeConstants do
      ...
      let(:line_number) { '196' }
      let(:cleaned_log_line) { "Accessing mime types via constants is deprecated. Please change `Mime::HTML` to `Mime[:html]`. (called from #{method} at #{file}:#{line_number})" }
      ...
      describe '#match?' do
        context 'when the log line is an access mime types via constants deprecation warning' do
          it 'matches' do
