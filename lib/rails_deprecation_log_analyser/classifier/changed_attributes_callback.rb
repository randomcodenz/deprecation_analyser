...
module RailsDeprecationLogAnalyser
  module Classifier
    class ChangedAttributesCallback < Base
      def match?(log_line)
        log_line.include?('The behavior of `changed_attributes` inside of after callbacks will be changing in the next version of Rails.')
      end
      ...
      def build_deprecation_warning(lines)
        warning = DeprecationWarning.new(
          ...
          summary: 'The behavior of `changed_attributes` inside of after callbacks will be changing in the next version of Rails.',
