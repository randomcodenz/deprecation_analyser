...
module RailsDeprecationLogAnalyser
  module Classifier
    class AttributeChangedCallback < Base
      def match?(log_line)
        log_line.include?('The behavior of `attribute_changed?` inside of after callbacks will be changing in the next version of Rails.')
      end
      ...
      def build_deprecation_warning(lines)
        warning = DeprecationWarning.new(
          ...
          summary: 'The behavior of `attribute_changed?` inside of after callbacks will be changing in the next version of Rails.',
